...
// ── Reset Chapter Inline Button (content area) ───────────────────────────────

interface ResetChapterInlineButtonProps {
  courseSlug: string
  allLessonSlugs: string[]
  chapterLessonSlugs: string[]
}

export function ResetChapterInlineButton({ courseSlug, allLessonSlugs, chapterLessonSlugs }: ResetChapterInlineButtonProps) {
  const { resetChapter, mounted, completed } = useProgress(courseSlug, allLessonSlugs)
  if (!mounted) return null
  const chapterHasProgress = chapterLessonSlugs.some(s => completed.includes(s))
  if (!chapterHasProgress) return null
  ...
  return (
    <button
      onClick={() => {
        if (window.confirm('Reset all lessons in this chapter? Other chapters are not affected.')) {
          resetChapter(chapterLessonSlugs)
          window.location.reload()
        }
      }}
