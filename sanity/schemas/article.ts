...
export default defineType({
  ...
  fields: [
    ...
    defineField({ name: 'seoTitle', title: 'SEO Title', type: 'string', group: 'seo', validation: (Rule) => Rule.max(60).warning('SEO titles over 60 characters may be truncated by search engines') }),
    defineField({ name: 'seoDescription', title: 'SEO Description', type: 'text', rows: 2, group: 'seo', validation: (Rule) => Rule.max(160).warning('SEO descriptions over 160 characters may be truncated by search engines') }),
    ...
    defineField({ name: 'contentId', title: 'Content ID', type: 'string', group: 'multisite', description: 'AI-generated content ID — format AB-ART-00001. Do not set on migrated WordPress content.' }),
    ...
    defineField({ name: 'eticpaLevel', title: 'ETICPA Level', type: 'string', group: 'abcm',
      description: 'Which ATQ level does this article belong to?',
      options: { list: [
        { title: 'Level 1 — Foundation Technician', value: 'level-1' },
        { title: 'Level 2 — Advanced Technician', value: 'level-2' },
      ], layout: 'radio' }
    }),
    defineField({ name: 'eticpaModule', title: 'ETICPA Module', type: 'string', group: 'abcm',
      description: 'Which module does this article belong to?',
      options: { list: [
        { title: 'Introduction to Accounting', value: 'introduction-to-accounting' },
        { title: 'Cost Accounting', value: 'cost-accounting' },
        { title: 'Business Skills', value: 'business-skills' },
        { title: 'Ethiopian Business Law', value: 'ethiopian-business-law' },
        { title: 'Financial Accounting', value: 'financial-accounting' },
        { title: 'Management Accounting', value: 'management-accounting' },
        { title: 'Assurance, Controls & Ethics', value: 'assurance-controls-ethics' },
        { title: 'Ethiopian Taxation', value: 'ethiopian-taxation' },
        { title: 'Ethiopian Public Sector Accounting', value: 'ethiopian-public-sector-accounting' },
      ], layout: 'radio' }
    }),
    defineField({ name: 'eticpaTopic', title: 'ETICPA Topic', type: 'string', group: 'abcm',
      description: 'The specific topic within the module. e.g. The Accounting Equation, Double Entry Bookkeeping',
    }),
