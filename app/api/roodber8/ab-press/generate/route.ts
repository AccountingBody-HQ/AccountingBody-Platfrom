            ...
            (lesson.linkedArticles || []).map(async (article: any) => {
              ...
              if (article.mcqUrl) {
                ...
                try {
                  ...
                  if (!practicePost) stats.mcqNotFound.push(practiceSlug)
                } catch {
                  quizQuestions = []
                  stats.mcqFailed.push(practiceSlug)
                }
              }
...
// Counts what Sanity provided vs what the book can contain, and surfaces
// anything dropped, empty, or unfetchable - by name.
function buildFidelity(course: any, stats: { mcqFailed: string[]; mcqNotFound: string[] }, bookType: string): string {
  ...
  const droppedTotal = Object.values(droppedTypes).reduce((a, b) => a + b, 0)
  const showQuestions = bookType === "combined" || bookType === "practice"
  const issues = emptyBodies + droppedTotal + stats.mcqFailed.length + stats.mcqNotFound.length
  const lines = [
    "CONTENT FIDELITY AUDIT",
    "======================",
    "",
    "Source inventory (fetched from Sanity):",
    "  Chapters: " + chapters.length,
    "  Lessons: " + lessons,
    "  Articles: " + articles,
    "  Articles with practice links (mcqUrl): " + mcqLinks,
  ...
  if (droppedTotal > 0) {
    lines.push("[WARN] " + droppedTotal + " non-text block(s) NOT rendered (renderer handles text blocks only):")
    for (const entry of Object.entries(droppedTypes)) lines.push("       - " + entry[0] + ": " + entry[1])
  } else {
    lines.push("[PASS] No non-text blocks dropped - every source block is renderable text")
  }
  if (stats.mcqNotFound.length > 0) {
    lines.push("[WARN] " + stats.mcqNotFound.length + " linked practice post(s) not found: " + stats.mcqNotFound.join(", "))
  } else {
    lines.push("[PASS] All linked practice posts found")
  }
  ...
  lines.push("")
  lines.push(issues === 0
    ? "FIDELITY VERDICT: COMPLETE - everything fetched from the source is accounted for in this book."
  ...
  try {
    ...
    const preflightBuffer = Buffer.from(
      buildPreflight(interiorPdf, coverPdf, pageCount, subtitle || course.title, bookType)
        + "\n\n"
        + buildFidelity(course, fidelityStats, bookType),
      "utf8"
    )
