...
export default function JobListingsClient({ isEthioTax }: { isEthioTax: boolean }) {
  // 'et' surfaces only jobs tagged for the EthioTax platform (African
  // markets ingested via lib/adzuna.ts's ADZUNA_MARKETS, plus any
  // EthioTax-specific employer listings); 'ab' (AccountingBody, the
  // default) surfaces every job regardless of market. Matches the
  // established codebase pattern — see app/jobs/find-work/page.tsx and
  // components/layout/NavigationWrapper.tsx, both of which derive this
  // same boolean from the `x-et-platform` header/cookie set in
  // middleware.ts — rather than inventing a new detection mechanism here.
  ...
  }, [platform, activeSearch, activeLocation, filters, page])
