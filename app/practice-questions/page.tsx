...
import { PracticeQuestionsPagination } from './PracticeQuestionsPagination'
import { buildPracticeQuestionsUrl } from './buildUrl'
...
export default async function PracticeQuestionsPage({
  ...
  // Build URL helper — preserves existing params and overrides one. The
  // actual logic lives in buildUrl.ts (framework-free) so it can also run
  // inside PracticeQuestionsPagination.tsx, a Client Component that can't
  // receive this as a closure directly — see that file for why.
  const filterState = { difficulty, search, letter, sort, category, page }
  function buildUrl(overrides: Record<string, string | number>) {
    return buildPracticeQuestionsUrl(filterState, overrides)
  }
  ...
  return (
    <div>
              ...
              {posts.length === 0 ? (
                ...
                <>
                  ...
                  {/* PAGINATION — the same shared component /jobs/listings
                      ...
                      as a real, crawlable <Link>, never a button.
                      PracticeQuestionsPagination is the small Client
                      Component that builds the hrefFor closure from this
                      (serializable) filter state — see that file for why
                      this Server Component can't pass a closure directly. */}
                  <PracticeQuestionsPagination
                    page={page}
      ...
      <JobsRecruitmentSection isEthioTax={isEthioTax} />
    </div>
  )
}
