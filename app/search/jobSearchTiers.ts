...
// Given the TOTAL counts already fetched for the phrase and and tiers
// (cheap count-only queries — see route.ts — run in parallel by the
// caller), decides which tier's rows/count to actually use. Takes plain
// totals, never row arrays, so there is no representation in which a
// caller could accidentally thread a countOnly response's empty `jobs: []`
// through here in place of its real total — see jobSearchTiers.test.ts for
// the regression case this guards, from the 9057937 production incident.
